...
| Number of subnets that can be shared with an account |  100 |  This is the quota for maximum number of subnets that can be shared with an AWS account\. To increase this quota contact AWS Support\. AWS recommends that you paginate your `DescribeSecurityGroups` and `DescribeSubnets` API calls before requesting an increase for this quota\.   | 